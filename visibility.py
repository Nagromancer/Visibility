...
class Observatory:
    ...
    def is_unobstructed(self, alt, az, times):
        ...
        altaz = SkyCoord(alt=alt, az=az, unit='deg', frame=AltAz(location=EarthLocation(lat=self.lat, lon=self.lon, height=self.height), obstime=times))
...
twist_min_alts = np.array([63, 63, 63, 62, 59, 53, 44, 35, 36, 36, 36, 35, 32, 30, 31, 37, 39, 40, 40, 40, 38, 36, 34, 23, 25, 27, 28, 30, 29,
     28, 26, 38, 50, 56, 59, 62, 63]) * u.deg

twist = Observatory("Twenty Inch Survey Telescope", 28.760255 * u.deg, -17.879284 * u.deg, 2348 * u.m, twist_min_alts, 6 * u.hourangle, -12 * u.deg)
ngts = Observatory("Next Generation Transit Survey", -24.615752 * u.deg, -70.391029 * u.deg, 2433 * u.m, 30 * u.deg, 5.3 * u.hourangle, -15 * u.deg)
tmo = Observatory("Tom Marsh Observatory", 52.376876 * u.deg, -1.567203 * u.deg, 93 * u.m, 30 * u.deg, 4.5 * u.hourangle, -12 * u.deg)
...
def main(args):
    ...
    try:
        ...
        print(f"Invalid observing date: {observing_date} (usage: --date dd/mm/yyyy)")
        exit()
    delta_midnight = np.linspace(-12 - obs.longitude_offset, 12 - obs.longitude_offset, 1441) * u.hour  # One minute intervals
    ...
    print(f"Night + Astronomical Twilight length:     {str(floor((astro_twilight_end - astro_twilight_start).value)).zfill(2)}h {str(floor(((astro_twilight_end - astro_twilight_start).value % 1) * 60)).zfill(2)}m"
    ...
    try:
        ...
        print(f"Night length: {' ' * 27} {str(floor((night_end - night_start).value)).zfill(2)}h {str(floor(((night_end - night_start).value % 1) * 60)).zfill(2)}m"
    ...
    if plot:
        ...
        plt.fill_between(delta_midnight, -90 * u.deg, 90 * u.deg, sunaltaz.alt < 0 * u.deg, color='#000000', zorder=0, alpha=0.3)
        plt.fill_between(delta_midnight, -90 * u.deg, 90 * u.deg, sunaltaz.alt < -6 * u.deg, color='#000000', zorder=0, alpha=0.45)
        plt.fill_between(delta_midnight, -90 * u.deg, 90 * u.deg, sunaltaz.alt < -12 * u.deg, color='#000000', zorder=0, alpha=0.6)
        plt.fill_between(delta_midnight, -90 * u.deg, 90 * u.deg, sunaltaz.alt < -18 * u.deg, color='#000000', zorder=0, alpha=0.9)

    obs_starts = []
    obs_ends = []
    for target, targaltaz, moon_sep, sky_coordinate, ss in zip(targets, targaltazs, moon_seps, sky_coordinates, solar_system):
        ...
        if np.any(above_min_alt & dark_time):
            ...
            print(f"{f'{target} ({ra:.5f}°, {dec:.5f}°):':<41} {str(floor((target_end - target_start).value*24)).zfill(2)}h" +
            ...
            if ss:
                ...
                if np.any(above_min_alt):
                    try:
                        ...
                        print(f'{f"{target} ({ra:.5f}°, {dec:.5f}°)":<41} is not visible on this night - unobstructed between {target_start.strftime("%H:%M")} and {target_end.strftime("%H:%M")}')
                    except IndexError:
        ...
        if plot:
            sc = plt.scatter(delta_midnight, targaltaz.alt, label=target, s=1, alpha=[1 if is_v and is_night else 0.2 for is_v, is_night in zip(visibility, dark_time)])
    ...
    if plot:
        ...
        plt.legend(handles=new_leg, bbox_to_anchor=(1, 1), loc='upper right', borderaxespad=0., handler_map={Line2D: HandlerLine2D(update_func=update_handle)})
...
if __name__ == '__main__':
    ...
    parser.add_argument('observatory', type=str, choices=['twist', 'ngts', 'tmo'], help='Observatory to calculate visibility for')
